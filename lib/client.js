...
function executeHandlers(sub, message) {
  let handlers = sub.handlers;
  let handler = 0;
  message.sub = sub;
  message.res = {};
  message.err = [];

  function nextHandler(message) {
    if (handlers[handler]) {
      return Promise.resolve(handlers[handler++](message))
        .then(nextHandler, (err) => {
          message.err.push(err);
          // Last handler is the return handler in this use-case TODO: Refactor
          return handlers[handlers.length - 1](message);
        });
...
Client.prototype._handleMessage = function (topic, payload, packet) {
  ...
  while (matched) {
    ...
    if (sub && !sub.cancelled) {
      ...
      if(Array.isArray(sub.handlers)) {
        executeHandlers(sub, message);
      } else {
        if (sub.handlers.length === 1) {
          sub.handlers.call(sub, message);
        } else {
          sub.handlers.call(sub, topic, payload, message);
        }
      }
    }
...
Client.prototype.subscribe = function (topic, handlers, options, cb) {
  const sub = new Subscription(this, topic, handlers);
  if (this.connected) {
